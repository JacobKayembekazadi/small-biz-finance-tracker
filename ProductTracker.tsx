...
const ProductSettingsModal: React.FC<{ onClose: () => void; product: Product; onSave: (product: Product) => void }> = ({ onClose, product, onSave }) => {
    const [tempProduct, setTempProduct] = useState(product);

    const handleChange = (key: keyof Product, value: string | number) => {
        setTempProduct({ ...tempProduct, [key]: key === 'name' || key === 'description' || key === 'color' || key === 'icon' ? value : (parseFloat(value as string) || 0) });
    };

    return (
        <Modal onClose={onClose}>
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-800">Product Settings</h2>
                <button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X size={24} /></button>
            </div>
            <div className="space-y-4">
                <FormInput label="Product Name" type="text" value={tempProduct.name} onChange={e => handleChange('name', e.target.value)} />
                <FormInput label="Initial Investment ($)" type="number" value={tempProduct.initialInvestment} onChange={e => handleChange('initialInvestment', e.target.value)} />
                <FormInput label="Cost per Unit ($)" type="number" value={tempProduct.unitCost} onChange={e => handleChange('unitCost', e.target.value)} />
                <FormInput label="Selling Price per Unit ($)" type="number" value={tempProduct.sellingPrice} onChange={e => handleChange('sellingPrice', e.target.value)} />
                <FormInput label="Initial Units" type="number" value={tempProduct.initialUnits} onChange={e => handleChange('initialUnits', e.target.value)} />
            </div>
            <div className="mt-6">
                <ActionButton onClick={() => { onSave(tempProduct); onClose(); }} className="bg-gray-700 text-white hover:bg-gray-800">Save Settings</ActionButton>
            </div>
        </Modal>
    );
};
...
export default function ProductTracker({ 
    ...
    const { initialUnits, unitsSold, inventory } = useMemo(() => {
        const unitsSold = transactions.reduce((acc, tx) => acc + tx.quantity, 0);
        const inventory = currentProduct.initialUnits - unitsSold;
        return { initialUnits: currentProduct.initialUnits, unitsSold, inventory };
    ...
    const handleAddSale = (sale: SaleData) => {
        const newTransaction: Transaction = {
            ...
            amount: sale.quantity * currentProduct.sellingPrice,
            type: 'sale'
        };
    ...
    const handleUpdateLog = (updatedItem: CombinedLogItem) => {
        if (updatedItem.type === 'sale') {
            const updatedTransactions = transactions.map(tx => tx.id === updatedItem.id ? updatedItem as Transaction : tx);
            setTransactions(updatedTransactions);
            onUpdateTransactions?.(updatedTransactions);
        } else {
            const updatedExpenses = expenses.map(ex => ex.id === updatedItem.id ? updatedItem as Expense : ex);
            setExpenses(updatedExpenses);
            onUpdateExpenses?.(updatedExpenses);
        }
        setEditingLog(null);
    ...
    const handleDeleteLog = (logItem: CombinedLogItem) => {
        if (window.confirm(`Are you sure you want to delete this ${logItem.type}? This action cannot be undone.`)) {
            if (logItem.type === 'sale') {
                const updatedTransactions = transactions.filter(tx => tx.id !== logItem.id);
                setTransactions(updatedTransactions);
                onUpdateTransactions?.(updatedTransactions);
            } else {
                const updatedExpenses = expenses.filter(ex => ex.id !== logItem.id);
                setExpenses(updatedExpenses);
                onUpdateExpenses?.(updatedExpenses);
            }
        }
    ...
    const financialData = useMemo(() => {
        ...
        const cogs = unitsSold * currentProduct.unitCost;
        const grossProfit = totalSales - cogs;
        const netProfit = grossProfit - totalExpenses - currentProduct.initialInvestment;
        const roi = currentProduct.initialInvestment > 0 ? (netProfit / currentProduct.initialInvestment) * 100 : 0;
        const profitMargin = totalSales > 0 ? (grossProfit / totalSales) * 100 : 0;

        return { totalSales, totalExpenses, cogs, grossProfit, netProfit, roi, profitMargin };
    }, [transactions, expenses, currentProduct, unitsSold]);

    const chartData = useMemo(() => {
        let cumulativeProfit = -currentProduct.initialInvestment;
        ...
        chronologicalLog.forEach((item) => {
            if (item.type === 'sale') {
                ...
                cumulativeProfit += (item.amount - (item.quantity * currentProduct.unitCost));
            } else {
        ...
        return data;
    }, [transactions, expenses, currentProduct]);

    return (
        <>
            {showSaleModal && <SaleModal onClose={() => setShowSaleModal(false)} onAddSale={handleAddSale} watchPrice={currentProduct.sellingPrice} inventory={inventory} />}
            {showExpenseModal && <ExpenseModal onClose={() => setShowExpenseModal(false)} onAddExpense={handleAddExpense} />}
            {showProductSettingsModal && <ProductSettingsModal onClose={() => setShowProductSettingsModal(false)} product={currentProduct} onSave={(updatedProduct) => { setCurrentProduct(updatedProduct); onUpdateProduct(updatedProduct); }} />}
            {editingLog && <EditLogModal logItem={editingLog} onClose={() => setEditingLog(null)} onSave={handleUpdateLog} watchPrice={currentProduct.sellingPrice} />}
                ...
                <div className="max-w-7xl mx-auto">
                    <header className="mb-8 flex justify-between items-center">
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                <h1 className="text-4xl font-bold text-gray-800">{currentProduct.name}</h1>
                                <p className="text-lg text-gray-500 mt-1">{currentProduct.description}</p>
                            </div>
                        </div>
                        <ActionButton onClick={() => setShowProductSettingsModal(true)} className="bg-white text-gray-700 hover:bg-gray-200 w-auto px-4">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div className="lg:col-span-1 space-y-8">
                            ...
                            <div className="space-y-4">
                               <StatCard title="Remaining Inventory" value={`${inventory} / ${initialUnits} units`} icon={<Package size={24} className="text-white"/>} color="bg-yellow-500" />
